<template>
  <div>
    <v-card id="scrolling-card" height="500px" elevation="2" style="padding: 50px; overflow-y: scroll">
  ...
  watch: {
    questionNo: {
      handler (newValue) {
        const idsRef = {
          0: 'lender-Namn',
          1: 'lender-Personnummer',
          2: 'lender-Adress',
          3: 'lender-Telefonnummer',
          4: 'lender-Email',
          5: 'borrower-Namn',
          6: 'borrower-Personnummer',
          7: 'borrower-Adress',
          8: 'borrower-Telefonnummer',
          9: 'borrower-Email',
          10: 'loan-amount',
          11: 'loan-amount-date',
          12: 'loan-amount-opts',
          ...
          16: 'loan-type',
          17: 'heShouldPay',
          18: 'shouldHeCancelLoan',
          19: 'paymentHolder'
        }

        if (idsRef[newValue]) {
          const m = document.getElementById(idsRef[newValue])
          const card = document.getElementById('scrolling-card')
          if (m) {
            const bodyRect = card.getBoundingClientRect()
            const elemRect = m.getBoundingClientRect()
            const offset = elemRect.top - bodyRect.top
            card.scrollTop = (card.scrollTop + offset)
            console.log(offset)
          }
        }
      }
